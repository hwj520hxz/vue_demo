  ...
  <div class="collect-data-maintenance">
    ...
    <div class="right-aside">
      <div class="simple-search-grid">
        ...
        <div class="tool-bar">
          <el-link icon="el-icon-document-add" @click="onAdd" v-show="!isUpdating && !isImporting" :disabled="!search.agenIdCode">新增</el-link>
        ...
        >
          <div slot="title" class="title">
            ...
            <span>uKey/机构证书发放维护 —— {{dialogMode.title}}</span>
          </div>
          <el-form :disabled="dialog.mode === 3" ref="form" :rules="dialog.rules" :model="dialog.data" label-width="130px">
            <el-row>
              <el-col :span="12">
            ...
            <el-row>
              <el-col :span="12">
                <el-form-item label="机构证书发放数" prop="issueNum">
                  <el-input
                    placeholder="请输入机构证书发放数"
                    v-model="dialog.data.issueNum"
                  ></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item label="uKey数" prop="ukeyNum">
                  <el-input
                    placeholder="请输入uKey数"
                    v-model="dialog.data.ukeyNum"
                  ></el-input>
                </el-form-item>
              </el-col>
            </el-row>
...
import { validateNumberNotEmpty } from '../../validator/validateNumberNotEmpty'
export default {
  name: '',
  components: {
    importDialog
  ...
  methods: {
    ...
    onTreeNodeClick (data) {
      if (data.children.length === 0 && data.id !== '-1') {
    ...
    async validateBeforeSaveOne () {
      ...
      let { data } = await this.$api.queryData({
        params: {
          mapperId: 'com.bosssoft.monitor.dao.centerReform.OrgIssueMapper.isExists',
          isList: false,
    ...
    async saveOne () {
      try {
        ...
        await this.$api.add({
          data: {
            mapperId: 'com.bosssoft.monitor.dao.centerReform.OrgIssueMapper.insertOrgIssueOne',
            ...this.dialog.data
          }
        })
    ...
    async save () {
      try {
        let model = this.grid.rows.filter(item => item.issueNum || item.ukeyNum)
        let orgIssues = JSON.stringify(model)
