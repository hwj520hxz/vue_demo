      ...
      <div class="simple-search-grid">
        ...
        <div class="grid-container">
          ...
          >
            ...
            <el-table-column prop="month" label="月份" align="center" show-overflow-tooltip>
              <template slot-scope="scope">
                <el-form v-if="isUpdating || isImporting" :model="scope.row" :rules="grid.rules" ref="month">
            ...
            <el-table-column prop="billBookedNum" align="center" label="门诊电子票据入账数" show-overflow-tooltip>
              <template slot-scope="scope">
                <el-form  v-if="isUpdating || isImporting" :model="scope.row" :rules="grid.rules" ref="billBookedNum">
                  <el-form-item prop="billBookedNum">
                    <el-input
                      v-model.trim="scope.row.billBookedNum"
                      clearable
                    ></el-input>
                  </el-form-item>
                </el-form>
                <span v-else>{{ scope.row.billBookedNum | emptyFilter}}</span>
              </template>
            </el-table-column>
            <el-table-column prop="hospitalBookedNum" align="center" label="住院电子票据入账数" show-overflow-tooltip>
              <template slot-scope="scope" >
                <el-form  v-if="isUpdating || isImporting" :model="scope.row" :rules="grid.rules" ref="hospitalBookedNum">
                  <el-form-item prop="hospitalBookedNum">
                    <el-input
                      v-model.trim="scope.row.hospitalBookedNum"
                      clearable
                    >
                    </el-input>
                  </el-form-item>
                </el-form>
                <span v-else>{{ scope.row.hospitalBookedNum | emptyFilter}}</span>
              </template>
            </el-table-column>
          </el-table>
        </div>
        ...
        >
          <div slot="title" class="title">
            ...
            <span>医疗入账数据维护 —— {{dialogMode.title}}</span>
          </div>
          <el-form :disabled="dialog.mode === 3" ref="form" :rules="dialog.rules" :model="dialog.data" label-width="150px">
            <el-row>
              <el-col :span="12">
            ...
            <el-row>
              <el-col :span="12">
                <el-form-item label="门诊电子票据入账数" prop="billBookedNum">
                  <el-input
                    clearable
                    placeholder="请输入电子票数"
                    v-model="dialog.data.billBookedNum"
                  ></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item label="住院电子票据入账数" prop="hospitalBookedNum">
                  <el-input
                    clearable
                    placeholder="请输入机打票数"
                    v-model="dialog.data.hospitalBookedNum"
                  ></el-input>
                </el-form-item>
              </el-col>
            </el-row>
...
export default {
  ...
  data () {
    ...
    return {
      templateConfig: {
        excelFileName: '医疗入账数据维护',
        excelHeader: ['月份', '门诊电子票据入账数', '住院电子票据入账数'],
        excelHeaderVal: ['month', 'billBookedNum', 'hospitalBookedNum'],
  ...
  methods: {
    ...
    async featchTreeData () {
      let { data } = await this.$api.queryData({
        params: {
          mapperId: 'com.bosssoft.monitor.dao.medicalReform.MedicalBookedMapper.queryRegion'
        }
      })
    ...
    async featchData () {
      ...
      let { data } = await this.$api.queryData({
        params: {
          mapperId: 'com.bosssoft.monitor.dao.medicalReform.MedicalBookedMapper.queryMedicalBooked',
          ...this.search
        }
      })
      ...
      for (let i = 1; i <= this.defaultMaxNum; i++) {
        ...
        for (let i = 0; i < data.length; i++) {
          if (moment(month).isSame(data[i].month)) {
            item.id = data[i].id
            item.billBookedNum = data[i].billBookedNum
            item.hospitalBookedNum = data[i].hospitalBookedNum
    ...
    formatAllRow (data) {
      ...
      for (let i = 0; i < data.length; i++) {
        if (moment(month).isSame(data[i].month)) {
          ...
          month = moment().subtract(monthSpan, 'months').format('YYYY-MM')
          let items = []
          while (!(moment(month).isSame(data[i]?.month)) && monthSpan < this.defaultMaxNum + 1) {
            let item = {
              id: '-1',
              month: month,
              billBookedNum: '',
    ...
    async validateBeforeSaveOne () {
      ...
      let { data } = await this.$api.queryData({
        params: {
          mapperId: 'com.bosssoft.monitor.dao.medicalReform.MedicalBookedMapper.isExists',
          isList: false,
          rgnCode: this.dialog.data.rgnCode,
          month: this.dialog.data.month
        }
      })
    ...
    async saveOne () {
      try {
        ...
        await this.$api.add({
          data: {
            mapperId: 'com.bosssoft.monitor.dao.medicalReform.MedicalBookedMapper.updateMedicalBooked',
            ...this.dialog.data
          }
        })
    ...
    async onSave () {
      try {
        await this.$refs['month'].validate()
        await this.$refs['billBookedNum'].validate()
        await this.$refs['hospitalBookedNum'].validate()
        let repeatCnt = 0
        let conflictCnt = 0
        this.grid.rows.forEach(item => {
          if (item.isConflict) {
    ...
    async save () {
      try {
        let model = this.grid.rows.filter(item => item.billBookedNum || item.hospitalBookedNum)
        let jsonData = JSON.stringify(model)
        let { data } = await this.$api.batchInsertMedicalBooked({
          headers: {
